...
import {
  ...
  findPressureCases
} from "../src/test-support/repoFamilyPressureAssertions.js";
...
test("Lane D keeps repo-agent seat exchange convention boundaries fail-closed", () => {
  ...
  const outboxBodyVisibilityIntakeCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_body_visibility_treated_as_result_intake_or_acceptance"
  );
  const outboxBodyVisibilityTruthCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_body_visibility_treated_as_report_truth_or_payload"
  );
  const outboxBodyVisibilityMutationCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_body_visibility_treated_as_application_merge_mutation_or_storage"
  );
  const outboxBodyVisibilityDispatchCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_body_visibility_treated_as_dispatch_execution_agent_or_platform"
  );
  const outboxBodyVisibilityAuthorityCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_body_visibility_treated_as_authority_event_auto_execute_or_enclosure"
  );
  const outboxBodyVisibilityTuiCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_body_visibility_tui_treated_as_action_authority"
  );
  ...
  const outboxResultIntakeReadinessCandidateCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_result_intake_readiness_treated_as_candidate_or_intake"
  );
  const outboxResultIntakeReadinessAcceptanceCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_result_intake_readiness_treated_as_acceptance_truth_or_payload"
  );
  const outboxResultIntakeReadinessMutationCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_result_intake_readiness_treated_as_application_merge_mutation_or_storage"
  );
  const outboxResultIntakeReadinessDispatchCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_result_intake_readiness_treated_as_dispatch_execution_agent_or_platform"
  );
  const outboxResultIntakeReadinessAuthorityCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_result_intake_readiness_treated_as_authority_event_auto_execute_or_enclosure"
  );
  const outboxResultIntakeReadinessTuiCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_result_intake_readiness_tui_treated_as_action_authority"
  );
  const outboxComplianceMeasurementImportCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_expectation_compliance_measurement_treated_as_import_intake_or_acceptance"
  );
  const outboxComplianceMeasurementTruthCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_expectation_compliance_measurement_treated_as_report_truth_or_payload"
  );
  const outboxComplianceMeasurementMutationCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_expectation_compliance_measurement_treated_as_application_merge_mutation_or_storage"
  );
  const outboxComplianceMeasurementDispatchCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_expectation_compliance_measurement_treated_as_dispatch_execution_agent_or_platform"
  );
  const outboxComplianceMeasurementAuthorityCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_expectation_compliance_measurement_treated_as_authority_autonomy_enclosure_or_auto_execute"
  );
  const outboxComplianceMeasurementTuiCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_expectation_compliance_measurement_tui_treated_as_action_authority"
  );
  const outboxRepairGuidanceImportCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_repair_guidance_treated_as_import_intake_or_acceptance"
  );
  const outboxRepairGuidanceAgentCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_repair_guidance_treated_as_agent_invocation_or_dispatch"
  );
  const outboxRepairGuidanceTruthCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_repair_guidance_treated_as_report_truth_or_payload"
  );
  const outboxRepairGuidanceMutationCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_repair_guidance_treated_as_application_merge_mutation_or_storage"
  );
  const outboxRepairGuidanceAuthorityCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_repair_guidance_treated_as_authority_event_auto_execute_or_enclosure"
  );
  const outboxRepairGuidanceTuiCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_repair_guidance_tui_treated_as_action_authority"
  );
  const outboxRepairGuidanceExportDeliveryCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_repair_guidance_export_view_treated_as_delivery_invocation_or_dispatch"
  );
  const outboxRepairGuidanceExportImportCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_repair_guidance_export_view_treated_as_import_intake_or_acceptance"
  );
  const outboxRepairGuidanceExportTruthCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_repair_guidance_export_view_treated_as_report_truth_or_payload"
  );
  const outboxRepairGuidanceExportMutationCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_repair_guidance_export_view_treated_as_application_merge_mutation_or_storage"
  );
  const outboxRepairGuidanceExportAuthorityCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_repair_guidance_export_view_treated_as_authority_event_auto_execute_or_enclosure"
  );
  const outboxRepairGuidanceExportTuiCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_repair_guidance_export_view_tui_treated_as_action_authority"
  );
  const outboxRepairRequestInboxDeliveryCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_repair_request_inbox_packet_candidate_treated_as_delivery_invocation_or_dispatch"
  );
  const outboxRepairRequestInboxImportCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_repair_request_inbox_packet_candidate_treated_as_import_intake_or_acceptance"
  );
  const outboxRepairRequestInboxTruthCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_repair_request_inbox_packet_candidate_treated_as_report_truth_or_payload"
  );
  const outboxRepairRequestInboxMutationCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_repair_request_inbox_packet_candidate_treated_as_application_merge_mutation_or_storage"
  );
  const outboxRepairRequestInboxAuthorityCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_repair_request_inbox_packet_candidate_treated_as_authority_event_auto_execute_or_enclosure"
  );
  const outboxRepairRequestInboxTuiCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_repair_request_inbox_packet_candidate_tui_treated_as_action_authority"
  );
  const outboxRepairRequestHandoffDeliveryCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_repair_request_handoff_operator_decision_treated_as_handoff_delivery_or_invocation"
  );
  const outboxRepairRequestHandoffImportCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_repair_request_handoff_operator_decision_treated_as_import_intake_or_acceptance"
  );
  const outboxRepairRequestHandoffTruthCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_repair_request_handoff_operator_decision_treated_as_report_truth_or_payload"
  );
  const outboxRepairRequestHandoffMutationCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_repair_request_handoff_operator_decision_treated_as_application_merge_mutation_or_storage"
  );
  const outboxRepairRequestHandoffAuthorityCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_outbox_report_repair_request_handoff_operator_decision_treated_as_authority_event_auto_execute_or_enclosure"
  );
  ...
  const repairedOutboxImportCandidateDecisionCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_import_candidate_treated_as_import_permission_or_decision"
  );
  const repairedOutboxImportCandidateImportCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_import_candidate_treated_as_report_import_or_result_intake"
  );
  const repairedOutboxImportCandidateTruthCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_import_candidate_treated_as_acceptance_truth_or_payload"
  );
  const repairedOutboxImportCandidateMutationCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_import_candidate_treated_as_application_merge_mutation_or_storage"
  );
  const repairedOutboxImportCandidateAuthorityCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_import_candidate_treated_as_dispatch_execution_authority_or_auto_execute"
  );
  const repairedOutboxImportCandidateTuiCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_import_candidate_tui_treated_as_action_authority"
  );
  const repairedOutboxImportDecisionImportCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_import_operator_decision_treated_as_report_import_or_result_intake"
  );
  const repairedOutboxImportDecisionTruthCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_import_operator_decision_treated_as_acceptance_truth_or_payload"
  );
  const repairedOutboxImportDecisionMutationCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_import_operator_decision_treated_as_application_merge_mutation_or_storage"
  );
  const repairedOutboxImportDecisionAuthorityCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_import_operator_decision_treated_as_dispatch_execution_authority_or_auto_execute"
  );
  const repairedOutboxImportDecisionProofCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_import_operator_decision_treated_as_candidate_or_handoff_receipt_proof"
  );
  const repairedOutboxImportDecisionTuiCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_import_operator_decision_tui_treated_as_action_authority"
  );
  const repairedOutboxImportObservationIntakeCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_import_observation_treated_as_result_intake_or_acceptance"
  );
  const repairedOutboxImportObservationTruthCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_import_observation_treated_as_report_truth_or_payload"
  );
  const repairedOutboxImportObservationMutationCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_import_observation_treated_as_application_merge_mutation_or_storage"
  );
  const repairedOutboxImportObservationAuthorityCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_import_observation_treated_as_dispatch_execution_authority_or_auto_execute"
  );
  const repairedOutboxImportObservationProofCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_import_observation_treated_as_delivery_read_execution_or_invocation_proof"
  );
  ...
  const repairedOutboxComplianceReviewIntakeCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_compliance_review_treated_as_result_intake_or_compliance_acceptance"
  );
  const repairedOutboxComplianceReviewTruthCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_compliance_review_treated_as_report_truth_or_payload"
  );
  const repairedOutboxComplianceReviewMutationCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_compliance_review_treated_as_application_merge_mutation_or_storage"
  );
  const repairedOutboxComplianceReviewAuthorityCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_compliance_review_treated_as_dispatch_execution_authority_or_auto_execute"
  );
  const repairedOutboxComplianceReviewProofCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_compliance_review_treated_as_agent_correctness_or_repair_success_proof"
  );
  const repairedOutboxComplianceReviewTuiCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_compliance_review_tui_treated_as_action_authority"
  );
  const repairedOutboxRepairEffectivenessIntakeCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_repair_effectiveness_measurement_treated_as_result_intake_or_acceptance"
  );
  const repairedOutboxRepairEffectivenessProofCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_repair_effectiveness_measurement_treated_as_repair_success_truth_or_agent_correctness"
  );
  const repairedOutboxRepairEffectivenessTruthCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_repair_effectiveness_measurement_treated_as_report_truth_or_payload"
  );
  const repairedOutboxRepairEffectivenessMutationCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_repair_effectiveness_measurement_treated_as_application_merge_mutation_or_storage"
  );
  const repairedOutboxRepairEffectivenessAuthorityCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_repair_effectiveness_measurement_treated_as_dispatch_execution_authority_or_auto_execute"
  );
  const repairedOutboxRepairEffectivenessTuiCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_repair_effectiveness_measurement_tui_treated_as_action_authority"
  );
  const repairedOutboxResultIntakeReadinessCandidateCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_result_intake_readiness_treated_as_candidate_or_intake"
  );
  const repairedOutboxResultIntakeReadinessTruthCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_result_intake_readiness_treated_as_compliance_acceptance_or_result_truth"
  );
  const repairedOutboxResultIntakeReadinessProofCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_result_intake_readiness_treated_as_repair_success_or_agent_correctness"
  );
  const repairedOutboxResultIntakeReadinessPayloadCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_result_intake_readiness_treated_as_payload_or_layer_truth"
  );
  const repairedOutboxResultIntakeReadinessMutationCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_result_intake_readiness_treated_as_application_merge_mutation_or_storage"
  );
  const repairedOutboxResultIntakeReadinessAuthorityCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_result_intake_readiness_treated_as_dispatch_execution_authority_or_auto_execute"
  );
  const repairedOutboxResultIntakeReadinessTuiCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_result_intake_readiness_tui_treated_as_action_authority"
  );
  const repairedOutboxResultIntakeCandidateDecisionCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_result_intake_candidate_treated_as_operator_decision_or_intake"
  );
  const repairedOutboxResultIntakeCandidateTruthCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_result_intake_candidate_treated_as_compliance_acceptance_or_result_truth"
  );
  const repairedOutboxResultIntakeCandidateProofCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_result_intake_candidate_treated_as_repair_success_or_agent_correctness"
  );
  const repairedOutboxResultIntakeCandidatePayloadCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_result_intake_candidate_treated_as_payload_or_layer_truth"
  );
  const repairedOutboxResultIntakeCandidateMutationCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_result_intake_candidate_treated_as_application_merge_mutation_or_storage"
  );
  const repairedOutboxResultIntakeCandidateAuthorityCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_result_intake_candidate_treated_as_dispatch_execution_authority_or_auto_execute"
  );
  const repairedOutboxResultIntakeCandidateTuiCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_result_intake_candidate_tui_treated_as_action_authority"
  );
  const repairedOutboxResultIntakeDecisionObservationCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_result_intake_operator_decision_treated_as_observation_or_intake"
  );
  const repairedOutboxResultIntakeDecisionTruthCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_result_intake_operator_decision_treated_as_compliance_acceptance_or_result_truth"
  );
  const repairedOutboxResultIntakeDecisionProofCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_result_intake_operator_decision_treated_as_repair_success_or_agent_correctness"
  );
  const repairedOutboxResultIntakeDecisionPayloadCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_result_intake_operator_decision_treated_as_payload_or_layer_truth"
  );
  const repairedOutboxResultIntakeDecisionMutationCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_result_intake_operator_decision_treated_as_application_merge_mutation_or_storage"
  );
  const repairedOutboxResultIntakeDecisionAuthorityCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_result_intake_operator_decision_treated_as_dispatch_execution_authority_or_auto_execute"
  );
  ...
  const repairedOutboxLoopSummaryTruthCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_loop_summary_treated_as_acceptance_or_truth"
  );
  const repairedOutboxLoopSummaryProofCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_loop_summary_treated_as_repair_success_or_agent_correctness"
  );
  const repairedOutboxLoopSummaryPayloadCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_loop_summary_treated_as_payload_layer_or_mutation"
  );
  const repairedOutboxLoopSummaryAuthorityCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_loop_summary_treated_as_dispatch_execution_authority_or_auto_execute"
  );
  const repairedOutboxLoopSummaryTuiCase = packet.cases.find((entry) =>
    entry.caseId === "repo_agent_repaired_outbox_report_loop_summary_tui_treated_as_action_authority"
  );
  const repairedResultIntakeLoopSummaryTruthCase = packet.cases.find((entry) =>
    entry.caseId === "repaired_outbox_report_result_intake_loop_summary_treated_as_acceptance_or_truth"
  );
  const repairedResultIntakeLoopSummaryProofCase = packet.cases.find((entry) =>
    entry.caseId === "repaired_outbox_report_result_intake_loop_summary_treated_as_repair_success_or_agent_correctness"
  );
  const repairedResultIntakeLoopSummaryPayloadCase = packet.cases.find((entry) =>
    entry.caseId === "repaired_outbox_report_result_intake_loop_summary_treated_as_payload_layer_or_mutation"
  );
  const repairedResultIntakeLoopSummaryAuthorityCase = packet.cases.find((entry) =>
    entry.caseId === "repaired_outbox_report_result_intake_loop_summary_treated_as_dispatch_execution_authority_or_auto_execute"
  );
  const repairedResultIntakeLoopSummaryTuiCase = packet.cases.find((entry) =>
    entry.caseId === "repaired_outbox_report_result_intake_loop_summary_tui_treated_as_action_authority"
  );
  const withRepairReadinessTruthCase = packet.cases.find((entry) =>
    entry.caseId === "tui_mediated_local_seat_loop_with_repair_readiness_treated_as_acceptance_or_truth"
  );
  const withRepairReadinessProofCase = packet.cases.find((entry) =>
    entry.caseId === "tui_mediated_local_seat_loop_with_repair_readiness_treated_as_repair_success_or_agent_correctness"
  );
  const withRepairReadinessPayloadCase = packet.cases.find((entry) =>
    entry.caseId === "tui_mediated_local_seat_loop_with_repair_readiness_treated_as_payload_layer_or_mutation"
  );
  const withRepairReadinessAuthorityCase = packet.cases.find((entry) =>
    entry.caseId === "tui_mediated_local_seat_loop_with_repair_readiness_treated_as_dispatch_execution_authority_or_auto_execute"
  );
  const withRepairReadinessEnclosureCase = packet.cases.find((entry) =>
    entry.caseId === "tui_mediated_local_seat_loop_with_repair_readiness_treated_as_enclosure_or_autonomy_proof"
  );
  const withRepairReadinessTuiCase = packet.cases.find((entry) =>
    entry.caseId === "tui_mediated_local_seat_loop_with_repair_readiness_tui_treated_as_action_authority"
  );
  const withRepairRepeatMeasurementTruthCase = packet.cases.find((entry) =>
    entry.caseId === "tui_mediated_local_seat_loop_with_repair_repeat_measurement_treated_as_acceptance_or_truth"
  );
  const withRepairRepeatMeasurementProofCase = packet.cases.find((entry) =>
    entry.caseId === "tui_mediated_local_seat_loop_with_repair_repeat_measurement_treated_as_repair_success_or_agent_correctness"
  );
  const withRepairRepeatMeasurementPayloadCase = packet.cases.find((entry) =>
    entry.caseId === "tui_mediated_local_seat_loop_with_repair_repeat_measurement_treated_as_payload_layer_or_mutation"
  );
  const withRepairRepeatMeasurementAuthorityCase = packet.cases.find((entry) =>
    entry.caseId === "tui_mediated_local_seat_loop_with_repair_repeat_measurement_treated_as_dispatch_execution_authority_or_auto_execute"
  );
  const withRepairRepeatMeasurementEnclosureCase = packet.cases.find((entry) =>
    entry.caseId === "tui_mediated_local_seat_loop_with_repair_repeat_measurement_treated_as_enclosure_or_autonomy_proof"
  );
  ...
  const nodeCapabilityCases = findPressureCases(packet, {
    endpoint:
      "edge_node_capability_profile_treated_as_endpoint_or_network_service",
    authority:
      "edge_node_capability_profile_treated_as_identity_admission_or_authority",
    truth:
      "edge_node_capability_profile_treated_as_report_truth_result_acceptance_or_payload_validity",
    mutation:
      "edge_node_capability_profile_treated_as_repo_layer_storage_mutation",
    execution:
      "edge_node_capability_profile_treated_as_scheduler_dispatch_execution_or_platform",
    enclosure:
      "edge_node_capability_profile_treated_as_full_enclosure_or_auto_execute",
    tui:
      "edge_node_capability_profile_tui_treated_as_action_authority"
  });
  const nodeProfileReadinessCases = findPressureCases(packet, {
    endpoint:
      "edge_node_profile_readiness_candidate_treated_as_endpoint_implementation",
    authority:
      "edge_node_profile_readiness_candidate_treated_as_identity_admission_or_authority",
    truth:
      "edge_node_profile_readiness_candidate_treated_as_report_truth_result_acceptance_or_payload_validity",
    mutation:
      "edge_node_profile_readiness_candidate_treated_as_repo_layer_storage_mutation",
    execution:
      "edge_node_profile_readiness_candidate_treated_as_scheduler_dispatch_execution_or_platform",
    enclosure:
      "edge_node_profile_readiness_candidate_treated_as_full_enclosure_or_auto_execute",
    tui:
      "edge_node_profile_readiness_candidate_tui_treated_as_action_authority"
  });
  const nodeProfileImplementationCases = findPressureCases(packet, {
    endpoint:
      "edge_node_profile_implementation_candidate_treated_as_implementation_or_endpoint",
    authority:
      "edge_node_profile_implementation_candidate_treated_as_identity_admission_or_authority",
    truth:
      "edge_node_profile_implementation_candidate_treated_as_report_truth_result_acceptance_or_payload_validity",
    mutation:
      "edge_node_profile_implementation_candidate_treated_as_repo_layer_storage_mutation",
    execution:
  ...
  const reportOnlyEndpointRuntimeCandidateCases = findPressureCases(packet, {
    endpoint:
      "edge_report_only_endpoint_runtime_candidate_treated_as_runtime_endpoint_or_transport",
    truth:
      "edge_report_only_endpoint_runtime_candidate_treated_as_report_truth_acceptance_or_payload_validity",
    mutation:
      "edge_report_only_endpoint_runtime_candidate_treated_as_repo_layer_storage_mutation",
    execution:
      "edge_report_only_endpoint_runtime_candidate_treated_as_scheduler_dispatch_execution_or_platform",
    authority:
      "edge_report_only_endpoint_runtime_candidate_treated_as_authority_enclosure_or_auto_execute",
    tui:
      "edge_report_only_endpoint_runtime_candidate_tui_treated_as_action_authority"
  });
  const reportOnlyEndpointRuntimeDecisionCases = findPressureCases(packet, {
    runtime:
      "edge_report_only_endpoint_runtime_decision_treated_as_planning_observation_or_runtime",
    truth:
      "edge_report_only_endpoint_runtime_decision_treated_as_report_truth_acceptance_or_payload_validity",
    mutation:
      "edge_report_only_endpoint_runtime_decision_treated_as_repo_layer_storage_mutation",
    execution:
      "edge_report_only_endpoint_runtime_decision_treated_as_scheduler_dispatch_execution_or_platform",
    authority:
      "edge_report_only_endpoint_runtime_decision_treated_as_authority_enclosure_or_auto_execute",
    tui:
      "edge_report_only_endpoint_runtime_decision_tui_treated_as_action_authority"
  });
  const reportOnlyEndpointRuntimePlanningObservationCases = findPressureCases(packet, {
    runtime:
      "edge_report_only_endpoint_runtime_planning_observation_treated_as_plan_or_runtime",
    truth:
      "edge_report_only_endpoint_runtime_planning_observation_treated_as_report_truth_acceptance_or_payload_validity",
    mutation:
      "edge_report_only_endpoint_runtime_planning_observation_treated_as_repo_layer_storage_mutation",
    execution:
      "edge_report_only_endpoint_runtime_planning_observation_treated_as_scheduler_dispatch_execution_or_platform",
    authority:
      "edge_report_only_endpoint_runtime_planning_observation_treated_as_authority_enclosure_or_auto_execute",
    tui:
      "edge_report_only_endpoint_runtime_planning_observation_tui_treated_as_action_authority"
  });
  const reportOnlyEndpointRuntimePlanCandidateCases = findPressureCases(packet, {
    runtime:
      "edge_report_only_endpoint_runtime_plan_candidate_treated_as_approved_plan_or_runtime",
    truth:
      "edge_report_only_endpoint_runtime_plan_candidate_treated_as_report_truth_acceptance_or_payload_validity",
    mutation:
      "edge_report_only_endpoint_runtime_plan_candidate_treated_as_repo_layer_storage_mutation",
    execution:
      "edge_report_only_endpoint_runtime_plan_candidate_treated_as_scheduler_dispatch_execution_or_platform",
    authority:
      "edge_report_only_endpoint_runtime_plan_candidate_treated_as_authority_enclosure_or_auto_execute",
    tui:
      "edge_report_only_endpoint_runtime_plan_candidate_tui_treated_as_action_authority"
  });
  const reportOnlyEndpointRuntimePlanDecisionCases = findPressureCases(packet, {
    runtime:
      "edge_report_only_endpoint_runtime_plan_decision_treated_as_preparation_observation_or_runtime",
    truth:
      "edge_report_only_endpoint_runtime_plan_decision_treated_as_report_truth_acceptance_or_payload_validity",
    mutation:
      "edge_report_only_endpoint_runtime_plan_decision_treated_as_repo_layer_storage_mutation",
    execution:
      "edge_report_only_endpoint_runtime_plan_decision_treated_as_scheduler_dispatch_execution_or_platform",
    authority:
      "edge_report_only_endpoint_runtime_plan_decision_treated_as_authority_enclosure_or_auto_execute",
    tui:
  ...
  const reportOnlyEndpointRuntimePreparationCandidateCases = findPressureCases(packet, {
    runtime:
      "edge_report_only_endpoint_runtime_preparation_candidate_treated_as_decision_or_preparation",
    truth:
      "edge_report_only_endpoint_runtime_preparation_candidate_treated_as_report_truth_acceptance_or_payload_validity",
    mutation:
      "edge_report_only_endpoint_runtime_preparation_candidate_treated_as_repo_layer_storage_mutation",
    execution:
      "edge_report_only_endpoint_runtime_preparation_candidate_treated_as_scheduler_dispatch_execution_or_platform",
    authority:
      "edge_report_only_endpoint_runtime_preparation_candidate_treated_as_authority_enclosure_or_auto_execute",
    tui:
      "edge_report_only_endpoint_runtime_preparation_candidate_tui_treated_as_action_authority"
  });
  const reportOnlyEndpointRuntimePreparationDecisionCases = findPressureCases(packet, {
    runtime:
      "edge_report_only_endpoint_runtime_preparation_decision_treated_as_preparation_observation_or_runtime",
    truth:
      "edge_report_only_endpoint_runtime_preparation_decision_treated_as_report_truth_acceptance_or_payload_validity",
    mutation:
      "edge_report_only_endpoint_runtime_preparation_decision_treated_as_repo_layer_storage_mutation",
    execution:
      "edge_report_only_endpoint_runtime_preparation_decision_treated_as_scheduler_dispatch_execution_or_platform",
    authority:
      "edge_report_only_endpoint_runtime_preparation_decision_treated_as_authority_enclosure_or_auto_execute",
    tui:
      "edge_report_only_endpoint_runtime_preparation_decision_tui_treated_as_action_authority"
  });
  const reportOnlyEndpointRuntimePreparationObservationCases = findPressureCases(packet, {
    runtime:
      "edge_report_only_endpoint_runtime_preparation_observation_treated_as_preparation_or_runtime",
    truth:
      "edge_report_only_endpoint_runtime_preparation_observation_treated_as_report_truth_acceptance_or_payload_validity",
    mutation:
      "edge_report_only_endpoint_runtime_preparation_observation_treated_as_repo_layer_storage_mutation",
    execution:
      "edge_report_only_endpoint_runtime_preparation_observation_treated_as_scheduler_dispatch_execution_or_platform",
    authority:
      "edge_report_only_endpoint_runtime_preparation_observation_treated_as_authority_enclosure_or_auto_execute",
    tui:
      "edge_report_only_endpoint_runtime_preparation_observation_tui_treated_as_action_authority"
  });
  const reportOnlyEndpointRuntimePacketCandidateCases = findPressureCases(packet, {
    runtime:
      "edge_report_only_endpoint_runtime_packet_candidate_treated_as_decision_or_runtime",
    truth:
      "edge_report_only_endpoint_runtime_packet_candidate_treated_as_report_truth_acceptance_or_payload_validity",
    mutation:
      "edge_report_only_endpoint_runtime_packet_candidate_treated_as_repo_layer_storage_mutation",
    execution:
      "edge_report_only_endpoint_runtime_packet_candidate_treated_as_scheduler_dispatch_execution_or_platform",
    authority:
      "edge_report_only_endpoint_runtime_packet_candidate_treated_as_authority_enclosure_or_auto_execute",
    tui:
      "edge_report_only_endpoint_runtime_packet_candidate_tui_treated_as_action_authority"
  });
  const tuiCase = packet.cases.find((entry) =>
  ...
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_body_visibility_treated_as_result_intake_or_acceptance"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_body_visibility_treated_as_report_truth_or_payload"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_body_visibility_treated_as_application_merge_mutation_or_storage"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_body_visibility_treated_as_dispatch_execution_agent_or_platform"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_body_visibility_treated_as_authority_event_auto_execute_or_enclosure"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_body_visibility_tui_treated_as_action_authority"),
    true
  );
  assert.equal(
  ...
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_result_intake_readiness_treated_as_candidate_or_intake"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_result_intake_readiness_treated_as_acceptance_truth_or_payload"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_result_intake_readiness_treated_as_application_merge_mutation_or_storage"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_result_intake_readiness_treated_as_dispatch_execution_agent_or_platform"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_result_intake_readiness_treated_as_authority_event_auto_execute_or_enclosure"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_result_intake_readiness_tui_treated_as_action_authority"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_expectation_compliance_measurement_treated_as_import_intake_or_acceptance"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_expectation_compliance_measurement_treated_as_report_truth_or_payload"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_expectation_compliance_measurement_treated_as_application_merge_mutation_or_storage"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_expectation_compliance_measurement_treated_as_dispatch_execution_agent_or_platform"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_expectation_compliance_measurement_treated_as_authority_autonomy_enclosure_or_auto_execute"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_expectation_compliance_measurement_tui_treated_as_action_authority"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_repair_guidance_treated_as_import_intake_or_acceptance"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_repair_guidance_treated_as_agent_invocation_or_dispatch"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_repair_guidance_treated_as_report_truth_or_payload"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_repair_guidance_treated_as_application_merge_mutation_or_storage"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_repair_guidance_treated_as_authority_event_auto_execute_or_enclosure"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_repair_guidance_tui_treated_as_action_authority"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_repair_guidance_export_view_treated_as_delivery_invocation_or_dispatch"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_repair_guidance_export_view_treated_as_import_intake_or_acceptance"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_repair_guidance_export_view_treated_as_report_truth_or_payload"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_repair_guidance_export_view_treated_as_application_merge_mutation_or_storage"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_repair_guidance_export_view_treated_as_authority_event_auto_execute_or_enclosure"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_repair_guidance_export_view_tui_treated_as_action_authority"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_repair_request_inbox_packet_candidate_treated_as_delivery_invocation_or_dispatch"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_repair_request_inbox_packet_candidate_treated_as_import_intake_or_acceptance"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_repair_request_inbox_packet_candidate_treated_as_report_truth_or_payload"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_repair_request_inbox_packet_candidate_treated_as_application_merge_mutation_or_storage"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_repair_request_inbox_packet_candidate_treated_as_authority_event_auto_execute_or_enclosure"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_repair_request_inbox_packet_candidate_tui_treated_as_action_authority"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_repair_request_handoff_operator_decision_treated_as_handoff_delivery_or_invocation"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_repair_request_handoff_operator_decision_treated_as_import_intake_or_acceptance"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_repair_request_handoff_operator_decision_treated_as_report_truth_or_payload"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_repair_request_handoff_operator_decision_treated_as_application_merge_mutation_or_storage"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_outbox_report_repair_request_handoff_operator_decision_treated_as_authority_event_auto_execute_or_enclosure"),
    true
  );
  assert.equal(
  ...
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_import_candidate_treated_as_import_permission_or_decision"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_import_candidate_treated_as_report_import_or_result_intake"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_import_candidate_treated_as_acceptance_truth_or_payload"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_import_candidate_treated_as_application_merge_mutation_or_storage"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_import_candidate_treated_as_dispatch_execution_authority_or_auto_execute"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_import_candidate_tui_treated_as_action_authority"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_import_operator_decision_treated_as_report_import_or_result_intake"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_import_operator_decision_treated_as_acceptance_truth_or_payload"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_import_operator_decision_treated_as_application_merge_mutation_or_storage"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_import_operator_decision_treated_as_dispatch_execution_authority_or_auto_execute"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_import_operator_decision_treated_as_candidate_or_handoff_receipt_proof"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_import_operator_decision_tui_treated_as_action_authority"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_import_observation_treated_as_result_intake_or_acceptance"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_import_observation_treated_as_report_truth_or_payload"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_import_observation_treated_as_application_merge_mutation_or_storage"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_import_observation_treated_as_dispatch_execution_authority_or_auto_execute"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_import_observation_treated_as_delivery_read_execution_or_invocation_proof"),
    true
  );
  assert.equal(
  ...
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_compliance_review_treated_as_result_intake_or_compliance_acceptance"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_compliance_review_treated_as_report_truth_or_payload"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_compliance_review_treated_as_application_merge_mutation_or_storage"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_compliance_review_treated_as_dispatch_execution_authority_or_auto_execute"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_compliance_review_treated_as_agent_correctness_or_repair_success_proof"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_compliance_review_tui_treated_as_action_authority"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_repair_effectiveness_measurement_treated_as_result_intake_or_acceptance"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_repair_effectiveness_measurement_treated_as_repair_success_truth_or_agent_correctness"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_repair_effectiveness_measurement_treated_as_report_truth_or_payload"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_repair_effectiveness_measurement_treated_as_application_merge_mutation_or_storage"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_repair_effectiveness_measurement_treated_as_dispatch_execution_authority_or_auto_execute"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_repair_effectiveness_measurement_tui_treated_as_action_authority"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_result_intake_readiness_treated_as_candidate_or_intake"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_result_intake_readiness_treated_as_compliance_acceptance_or_result_truth"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_result_intake_readiness_treated_as_repair_success_or_agent_correctness"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_result_intake_readiness_treated_as_payload_or_layer_truth"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_result_intake_readiness_treated_as_application_merge_mutation_or_storage"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_result_intake_readiness_treated_as_dispatch_execution_authority_or_auto_execute"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_result_intake_readiness_tui_treated_as_action_authority"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_result_intake_candidate_treated_as_operator_decision_or_intake"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_result_intake_candidate_treated_as_compliance_acceptance_or_result_truth"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_result_intake_candidate_treated_as_repair_success_or_agent_correctness"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_result_intake_candidate_treated_as_payload_or_layer_truth"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_result_intake_candidate_treated_as_application_merge_mutation_or_storage"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_result_intake_candidate_treated_as_dispatch_execution_authority_or_auto_execute"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_result_intake_candidate_tui_treated_as_action_authority"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_result_intake_operator_decision_treated_as_observation_or_intake"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_result_intake_operator_decision_treated_as_compliance_acceptance_or_result_truth"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_result_intake_operator_decision_treated_as_repair_success_or_agent_correctness"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_result_intake_operator_decision_treated_as_payload_or_layer_truth"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_result_intake_operator_decision_treated_as_application_merge_mutation_or_storage"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_result_intake_operator_decision_treated_as_dispatch_execution_authority_or_auto_execute"),
    true
  );
  assert.equal(
  ...
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_loop_summary_treated_as_acceptance_or_truth"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_loop_summary_treated_as_repair_success_or_agent_correctness"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_loop_summary_treated_as_payload_layer_or_mutation"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_loop_summary_treated_as_dispatch_execution_authority_or_auto_execute"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repo_agent_repaired_outbox_report_loop_summary_tui_treated_as_action_authority"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repaired_outbox_report_result_intake_loop_summary_treated_as_acceptance_or_truth"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repaired_outbox_report_result_intake_loop_summary_treated_as_repair_success_or_agent_correctness"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repaired_outbox_report_result_intake_loop_summary_treated_as_payload_layer_or_mutation"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repaired_outbox_report_result_intake_loop_summary_treated_as_dispatch_execution_authority_or_auto_execute"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("repaired_outbox_report_result_intake_loop_summary_tui_treated_as_action_authority"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("tui_mediated_local_seat_loop_with_repair_readiness_treated_as_acceptance_or_truth"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("tui_mediated_local_seat_loop_with_repair_readiness_treated_as_repair_success_or_agent_correctness"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("tui_mediated_local_seat_loop_with_repair_readiness_treated_as_payload_layer_or_mutation"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("tui_mediated_local_seat_loop_with_repair_readiness_treated_as_dispatch_execution_authority_or_auto_execute"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("tui_mediated_local_seat_loop_with_repair_readiness_treated_as_enclosure_or_autonomy_proof"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("tui_mediated_local_seat_loop_with_repair_readiness_tui_treated_as_action_authority"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("tui_mediated_local_seat_loop_with_repair_repeat_measurement_treated_as_acceptance_or_truth"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("tui_mediated_local_seat_loop_with_repair_repeat_measurement_treated_as_repair_success_or_agent_correctness"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("tui_mediated_local_seat_loop_with_repair_repeat_measurement_treated_as_payload_layer_or_mutation"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("tui_mediated_local_seat_loop_with_repair_repeat_measurement_treated_as_dispatch_execution_authority_or_auto_execute"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("tui_mediated_local_seat_loop_with_repair_repeat_measurement_treated_as_enclosure_or_autonomy_proof"),
    true
  );
  assert.equal(
    report.blockedCaseIds.includes("tui_mediated_local_seat_loop_with_repair_repeat_measurement_tui_treated_as_action_authority"),
    true
  );
  ...
  assert.equal(outboxRepairGuidanceImportCase.boundary.claimsTruth, false);
  assert.equal(outboxRepairGuidanceImportCase.boundary.createsAuthority, false);
  assert.equal(outboxRepairGuidanceAgentCase.stopStatus, "blocked");
  assert.equal(outboxRepairGuidanceAgentCase.boundary.dispatchesRepoAgents, false);
  assert.equal(outboxRepairGuidanceAgentCase.boundary.executesBehavior, false);
  assert.equal(outboxRepairGuidanceAgentCase.boundary.callsPlatform, false);
  assert.equal(outboxRepairGuidanceTruthCase.stopStatus, "blocked");
  assert.equal(outboxRepairGuidanceTruthCase.boundary.claimsTruth, false);
  assert.equal(outboxRepairGuidanceTruthCase.boundary.claimsPayloadValidity, false);
  assert.equal(outboxRepairGuidanceTruthCase.boundary.mutatesLayer, false);
  assert.equal(outboxRepairGuidanceMutationCase.stopStatus, "blocked");
  assert.equal(outboxRepairGuidanceMutationCase.boundary.mutatesSourceRepo, false);
  assert.equal(outboxRepairGuidanceMutationCase.boundary.mutatesLayer, false);
  assert.equal(outboxRepairGuidanceMutationCase.boundary.writesProductionStorage, false);
  assert.equal(outboxRepairGuidanceAuthorityCase.stopStatus, "blocked");
  assert.equal(outboxRepairGuidanceAuthorityCase.boundary.createsAuthority, false);
  assert.equal(outboxRepairGuidanceAuthorityCase.boundary.autoExecutes, false);
  assert.equal(outboxRepairGuidanceTuiCase.stopStatus, "blocked");
  assert.equal(outboxRepairGuidanceTuiCase.boundary.callsEdge, false);
  assert.equal(outboxRepairGuidanceTuiCase.boundary.dispatchesRepoAgents, false);
  assert.equal(outboxRepairGuidanceTuiCase.boundary.createsAuthority, false);
  assert.equal(outboxRepairGuidanceExportDeliveryCase.stopStatus, "blocked");
  assert.equal(outboxRepairGuidanceExportDeliveryCase.boundary.dispatchesRepoAgents, false);
  assert.equal(outboxRepairGuidanceExportDeliveryCase.boundary.executesBehavior, false);
  assert.equal(outboxRepairGuidanceExportDeliveryCase.boundary.callsPlatform, false);
  assert.equal(outboxRepairGuidanceExportImportCase.stopStatus, "blocked");
  assert.equal(outboxRepairGuidanceExportImportCase.boundary.callsEdge, false);
  assert.equal(outboxRepairGuidanceExportImportCase.boundary.claimsTruth, false);
  assert.equal(outboxRepairGuidanceExportImportCase.boundary.createsAuthority, false);
  assert.equal(outboxRepairGuidanceExportTruthCase.stopStatus, "blocked");
  assert.equal(outboxRepairGuidanceExportTruthCase.boundary.claimsTruth, false);
  assert.equal(outboxRepairGuidanceExportTruthCase.boundary.claimsPayloadValidity, false);
  assert.equal(outboxRepairGuidanceExportTruthCase.boundary.mutatesLayer, false);
  assert.equal(outboxRepairGuidanceExportMutationCase.stopStatus, "blocked");
  assert.equal(outboxRepairGuidanceExportMutationCase.boundary.mutatesSourceRepo, false);
  assert.equal(outboxRepairGuidanceExportMutationCase.boundary.mutatesLayer, false);
  assert.equal(outboxRepairGuidanceExportMutationCase.boundary.writesProductionStorage, false);
  assert.equal(outboxRepairGuidanceExportAuthorityCase.stopStatus, "blocked");
  assert.equal(outboxRepairGuidanceExportAuthorityCase.boundary.createsAuthority, false);
  assert.equal(outboxRepairGuidanceExportAuthorityCase.boundary.autoExecutes, false);
  assert.equal(outboxRepairGuidanceExportTuiCase.stopStatus, "blocked");
  assert.equal(outboxRepairGuidanceExportTuiCase.boundary.callsEdge, false);
  assert.equal(outboxRepairGuidanceExportTuiCase.boundary.dispatchesRepoAgents, false);
  assert.equal(outboxRepairGuidanceExportTuiCase.boundary.createsAuthority, false);
  assert.equal(outboxRepairRequestInboxDeliveryCase.stopStatus, "blocked");
  assert.equal(outboxRepairRequestInboxDeliveryCase.boundary.dispatchesRepoAgents, false);
  assert.equal(outboxRepairRequestInboxDeliveryCase.boundary.executesBehavior, false);
  assert.equal(outboxRepairRequestInboxDeliveryCase.boundary.callsPlatform, false);
  assert.equal(outboxRepairRequestInboxImportCase.stopStatus, "blocked");
  assert.equal(outboxRepairRequestInboxImportCase.boundary.callsEdge, false);
  assert.equal(outboxRepairRequestInboxImportCase.boundary.claimsTruth, false);
  assert.equal(outboxRepairRequestInboxImportCase.boundary.createsAuthority, false);
  assert.equal(outboxRepairRequestInboxTruthCase.stopStatus, "blocked");
  assert.equal(outboxRepairRequestInboxTruthCase.boundary.claimsTruth, false);
  assert.equal(outboxRepairRequestInboxTruthCase.boundary.claimsPayloadValidity, false);
  assert.equal(outboxRepairRequestInboxTruthCase.boundary.mutatesLayer, false);
  assert.equal(outboxRepairRequestInboxMutationCase.stopStatus, "blocked");
  assert.equal(outboxRepairRequestInboxMutationCase.boundary.mutatesSourceRepo, false);
  assert.equal(outboxRepairRequestInboxMutationCase.boundary.mutatesLayer, false);
  assert.equal(outboxRepairRequestInboxMutationCase.boundary.writesProductionStorage, false);
  assert.equal(outboxRepairRequestInboxAuthorityCase.stopStatus, "blocked");
  assert.equal(outboxRepairRequestInboxAuthorityCase.boundary.createsAuthority, false);
  assert.equal(outboxRepairRequestInboxAuthorityCase.boundary.autoExecutes, false);
  assert.equal(outboxRepairRequestInboxTuiCase.stopStatus, "blocked");
  assert.equal(outboxRepairRequestInboxTuiCase.boundary.callsEdge, false);
  assert.equal(outboxRepairRequestInboxTuiCase.boundary.dispatchesRepoAgents, false);
  assert.equal(outboxRepairRequestInboxTuiCase.boundary.createsAuthority, false);
  assert.equal(outboxRepairRequestHandoffDeliveryCase.stopStatus, "blocked");
  assert.equal(outboxRepairRequestHandoffDeliveryCase.boundary.dispatchesRepoAgents, false);
  assert.equal(outboxRepairRequestHandoffDeliveryCase.boundary.executesBehavior, false);
  assert.equal(outboxRepairRequestHandoffDeliveryCase.boundary.callsPlatform, false);
  assert.equal(outboxRepairRequestHandoffImportCase.stopStatus, "blocked");
  assert.equal(outboxRepairRequestHandoffImportCase.boundary.callsEdge, false);
  assert.equal(outboxRepairRequestHandoffImportCase.boundary.claimsTruth, false);
  assert.equal(outboxRepairRequestHandoffImportCase.boundary.createsAuthority, false);
  assert.equal(outboxRepairRequestHandoffTruthCase.stopStatus, "blocked");
  assert.equal(outboxRepairRequestHandoffTruthCase.boundary.claimsTruth, false);
  assert.equal(outboxRepairRequestHandoffTruthCase.boundary.claimsPayloadValidity, false);
  assert.equal(outboxRepairRequestHandoffTruthCase.boundary.mutatesLayer, false);
  assert.equal(outboxRepairRequestHandoffMutationCase.stopStatus, "blocked");
  assert.equal(outboxRepairRequestHandoffMutationCase.boundary.mutatesSourceRepo, false);
  assert.equal(outboxRepairRequestHandoffMutationCase.boundary.mutatesLayer, false);
  assert.equal(outboxRepairRequestHandoffMutationCase.boundary.writesProductionStorage, false);
  assert.equal(outboxRepairRequestHandoffAuthorityCase.stopStatus, "blocked");
  assert.equal(outboxRepairRequestHandoffAuthorityCase.boundary.createsAuthority, false);
  assert.equal(outboxRepairRequestHandoffAuthorityCase.boundary.autoExecutes, false);
  assert.equal(outboxRepairRequestHandoffTuiCase.stopStatus, "blocked");
  ...
  assert.equal(repairedOutboxImportCandidateDecisionCase.stopStatus, "blocked");
  assert.equal(repairedOutboxImportCandidateDecisionCase.boundary.callsEdge, false);
  assert.equal(repairedOutboxImportCandidateDecisionCase.boundary.createsAuthority, false);
  assert.equal(repairedOutboxImportCandidateImportCase.stopStatus, "blocked");
  assert.equal(repairedOutboxImportCandidateImportCase.boundary.callsEdge, false);
  assert.equal(repairedOutboxImportCandidateImportCase.boundary.claimsTruth, false);
  assert.equal(repairedOutboxImportCandidateImportCase.boundary.createsAuthority, false);
  assert.equal(repairedOutboxImportCandidateTruthCase.stopStatus, "blocked");
  assert.equal(repairedOutboxImportCandidateTruthCase.boundary.claimsTruth, false);
  assert.equal(repairedOutboxImportCandidateTruthCase.boundary.claimsPayloadValidity, false);
  assert.equal(repairedOutboxImportCandidateTruthCase.boundary.mutatesLayer, false);
  assert.equal(repairedOutboxImportCandidateMutationCase.stopStatus, "blocked");
  assert.equal(repairedOutboxImportCandidateMutationCase.boundary.mutatesSourceRepo, false);
  assert.equal(repairedOutboxImportCandidateMutationCase.boundary.mutatesLayer, false);
  assert.equal(repairedOutboxImportCandidateMutationCase.boundary.writesProductionStorage, false);
  assert.equal(repairedOutboxImportCandidateAuthorityCase.stopStatus, "blocked");
  assert.equal(repairedOutboxImportCandidateAuthorityCase.boundary.dispatchesRepoAgents, false);
  assert.equal(repairedOutboxImportCandidateAuthorityCase.boundary.executesBehavior, false);
  assert.equal(repairedOutboxImportCandidateAuthorityCase.boundary.callsPlatform, false);
  assert.equal(repairedOutboxImportCandidateAuthorityCase.boundary.createsAuthority, false);
  assert.equal(repairedOutboxImportCandidateTuiCase.stopStatus, "blocked");
  assert.equal(repairedOutboxImportCandidateTuiCase.boundary.callsEdge, false);
  assert.equal(repairedOutboxImportCandidateTuiCase.boundary.dispatchesRepoAgents, false);
  assert.equal(repairedOutboxImportCandidateTuiCase.boundary.createsAuthority, false);
  assert.equal(repairedOutboxImportDecisionImportCase.stopStatus, "blocked");
  assert.equal(repairedOutboxImportDecisionImportCase.boundary.callsEdge, false);
  assert.equal(repairedOutboxImportDecisionImportCase.boundary.claimsTruth, false);
  assert.equal(repairedOutboxImportDecisionImportCase.boundary.createsAuthority, false);
  assert.equal(repairedOutboxImportDecisionTruthCase.stopStatus, "blocked");
  assert.equal(repairedOutboxImportDecisionTruthCase.boundary.claimsTruth, false);
  assert.equal(repairedOutboxImportDecisionTruthCase.boundary.claimsPayloadValidity, false);
  assert.equal(repairedOutboxImportDecisionTruthCase.boundary.mutatesLayer, false);
  assert.equal(repairedOutboxImportDecisionMutationCase.stopStatus, "blocked");
  assert.equal(repairedOutboxImportDecisionMutationCase.boundary.mutatesSourceRepo, false);
  assert.equal(repairedOutboxImportDecisionMutationCase.boundary.mutatesLayer, false);
  assert.equal(repairedOutboxImportDecisionMutationCase.boundary.writesProductionStorage, false);
  assert.equal(repairedOutboxImportDecisionAuthorityCase.stopStatus, "blocked");
  assert.equal(repairedOutboxImportDecisionAuthorityCase.boundary.dispatchesRepoAgents, false);
  assert.equal(repairedOutboxImportDecisionAuthorityCase.boundary.executesBehavior, false);
  assert.equal(repairedOutboxImportDecisionAuthorityCase.boundary.callsPlatform, false);
  assert.equal(repairedOutboxImportDecisionAuthorityCase.boundary.createsAuthority, false);
  assert.equal(repairedOutboxImportDecisionProofCase.stopStatus, "blocked");
  assert.equal(repairedOutboxImportDecisionProofCase.boundary.claimsTruth, false);
  assert.equal(repairedOutboxImportDecisionProofCase.boundary.dispatchesRepoAgents, false);
  assert.equal(repairedOutboxImportDecisionProofCase.boundary.executesBehavior, false);
  assert.equal(repairedOutboxImportDecisionTuiCase.stopStatus, "blocked");
  assert.equal(repairedOutboxImportDecisionTuiCase.boundary.callsEdge, false);
  assert.equal(repairedOutboxImportDecisionTuiCase.boundary.dispatchesRepoAgents, false);
  assert.equal(repairedOutboxImportDecisionTuiCase.boundary.createsAuthority, false);
  assert.equal(repairedOutboxImportObservationIntakeCase.stopStatus, "blocked");
  assert.equal(repairedOutboxImportObservationIntakeCase.boundary.callsEdge, false);
  assert.equal(repairedOutboxImportObservationIntakeCase.boundary.claimsTruth, false);
  assert.equal(repairedOutboxImportObservationIntakeCase.boundary.createsAuthority, false);
  assert.equal(repairedOutboxImportObservationTruthCase.stopStatus, "blocked");
  assert.equal(repairedOutboxImportObservationTruthCase.boundary.claimsTruth, false);
  assert.equal(repairedOutboxImportObservationTruthCase.boundary.claimsPayloadValidity, false);
  assert.equal(repairedOutboxImportObservationTruthCase.boundary.mutatesLayer, false);
  assert.equal(repairedOutboxImportObservationMutationCase.stopStatus, "blocked");
  assert.equal(repairedOutboxImportObservationMutationCase.boundary.mutatesSourceRepo, false);
  assert.equal(repairedOutboxImportObservationMutationCase.boundary.mutatesLayer, false);
  assert.equal(repairedOutboxImportObservationMutationCase.boundary.writesProductionStorage, false);
  assert.equal(repairedOutboxImportObservationAuthorityCase.stopStatus, "blocked");
  assert.equal(repairedOutboxImportObservationAuthorityCase.boundary.dispatchesRepoAgents, false);
  assert.equal(repairedOutboxImportObservationAuthorityCase.boundary.executesBehavior, false);
  assert.equal(repairedOutboxImportObservationAuthorityCase.boundary.callsPlatform, false);
  assert.equal(repairedOutboxImportObservationAuthorityCase.boundary.createsAuthority, false);
  assert.equal(repairedOutboxImportObservationProofCase.stopStatus, "blocked");
  assert.equal(repairedOutboxImportObservationProofCase.boundary.claimsTruth, false);
  assert.equal(repairedOutboxImportObservationProofCase.boundary.dispatchesRepoAgents, false);
  assert.equal(repairedOutboxImportObservationProofCase.boundary.executesBehavior, false);
  assert.equal(repairedOutboxImportObservationTuiCase.stopStatus, "blocked");
  ...
  assert.equal(repairedOutboxComplianceReviewIntakeCase.stopStatus, "blocked");
  assert.equal(repairedOutboxComplianceReviewIntakeCase.boundary.callsEdge, false);
  assert.equal(repairedOutboxComplianceReviewIntakeCase.boundary.claimsTruth, false);
  assert.equal(repairedOutboxComplianceReviewIntakeCase.boundary.createsAuthority, false);
  assert.equal(repairedOutboxComplianceReviewTruthCase.stopStatus, "blocked");
  assert.equal(repairedOutboxComplianceReviewTruthCase.boundary.claimsTruth, false);
  assert.equal(repairedOutboxComplianceReviewTruthCase.boundary.claimsPayloadValidity, false);
  assert.equal(repairedOutboxComplianceReviewTruthCase.boundary.mutatesLayer, false);
  assert.equal(repairedOutboxComplianceReviewMutationCase.stopStatus, "blocked");
  assert.equal(repairedOutboxComplianceReviewMutationCase.boundary.mutatesSourceRepo, false);
  assert.equal(repairedOutboxComplianceReviewMutationCase.boundary.mutatesLayer, false);
  assert.equal(repairedOutboxComplianceReviewMutationCase.boundary.writesProductionStorage, false);
  assert.equal(repairedOutboxComplianceReviewAuthorityCase.stopStatus, "blocked");
  assert.equal(repairedOutboxComplianceReviewAuthorityCase.boundary.dispatchesRepoAgents, false);
  assert.equal(repairedOutboxComplianceReviewAuthorityCase.boundary.executesBehavior, false);
  assert.equal(repairedOutboxComplianceReviewAuthorityCase.boundary.callsPlatform, false);
  assert.equal(repairedOutboxComplianceReviewAuthorityCase.boundary.createsAuthority, false);
  assert.equal(repairedOutboxComplianceReviewProofCase.stopStatus, "blocked");
  assert.equal(repairedOutboxComplianceReviewProofCase.boundary.claimsTruth, false);
  assert.equal(repairedOutboxComplianceReviewProofCase.boundary.dispatchesRepoAgents, false);
  assert.equal(repairedOutboxComplianceReviewProofCase.boundary.executesBehavior, false);
  assert.equal(repairedOutboxComplianceReviewTuiCase.stopStatus, "blocked");
  assert.equal(repairedOutboxComplianceReviewTuiCase.boundary.callsEdge, false);
  assert.equal(repairedOutboxComplianceReviewTuiCase.boundary.dispatchesRepoAgents, false);
  assert.equal(repairedOutboxComplianceReviewTuiCase.boundary.createsAuthority, false);
  assert.equal(repairedOutboxRepairEffectivenessIntakeCase.stopStatus, "blocked");
  assert.equal(repairedOutboxRepairEffectivenessIntakeCase.boundary.callsEdge, false);
  assert.equal(repairedOutboxRepairEffectivenessIntakeCase.boundary.claimsTruth, false);
  assert.equal(repairedOutboxRepairEffectivenessIntakeCase.boundary.createsAuthority, false);
  assert.equal(repairedOutboxRepairEffectivenessProofCase.stopStatus, "blocked");
  assert.equal(repairedOutboxRepairEffectivenessProofCase.boundary.claimsTruth, false);
  assert.equal(repairedOutboxRepairEffectivenessProofCase.boundary.dispatchesRepoAgents, false);
  assert.equal(repairedOutboxRepairEffectivenessProofCase.boundary.executesBehavior, false);
  assert.equal(repairedOutboxRepairEffectivenessTruthCase.stopStatus, "blocked");
  assert.equal(repairedOutboxRepairEffectivenessTruthCase.boundary.claimsTruth, false);
  assert.equal(repairedOutboxRepairEffectivenessTruthCase.boundary.claimsPayloadValidity, false);
  assert.equal(repairedOutboxRepairEffectivenessTruthCase.boundary.mutatesLayer, false);
  assert.equal(repairedOutboxRepairEffectivenessMutationCase.stopStatus, "blocked");
  assert.equal(repairedOutboxRepairEffectivenessMutationCase.boundary.mutatesSourceRepo, false);
  assert.equal(repairedOutboxRepairEffectivenessMutationCase.boundary.mutatesLayer, false);
  assert.equal(repairedOutboxRepairEffectivenessMutationCase.boundary.writesProductionStorage, false);
  assert.equal(repairedOutboxRepairEffectivenessAuthorityCase.stopStatus, "blocked");
  assert.equal(repairedOutboxRepairEffectivenessAuthorityCase.boundary.dispatchesRepoAgents, false);
  assert.equal(repairedOutboxRepairEffectivenessAuthorityCase.boundary.executesBehavior, false);
  assert.equal(repairedOutboxRepairEffectivenessAuthorityCase.boundary.callsPlatform, false);
  assert.equal(repairedOutboxRepairEffectivenessAuthorityCase.boundary.createsAuthority, false);
  assert.equal(repairedOutboxRepairEffectivenessTuiCase.stopStatus, "blocked");
  assert.equal(repairedOutboxRepairEffectivenessTuiCase.boundary.callsEdge, false);
  assert.equal(repairedOutboxRepairEffectivenessTuiCase.boundary.dispatchesRepoAgents, false);
  assert.equal(repairedOutboxRepairEffectivenessTuiCase.boundary.createsAuthority, false);
  assert.equal(repairedOutboxResultIntakeReadinessCandidateCase.stopStatus, "blocked");
  assert.equal(repairedOutboxResultIntakeReadinessCandidateCase.boundary.callsEdge, false);
  assert.equal(repairedOutboxResultIntakeReadinessCandidateCase.boundary.claimsTruth, false);
  assert.equal(repairedOutboxResultIntakeReadinessCandidateCase.boundary.createsAuthority, false);
  assert.equal(repairedOutboxResultIntakeReadinessTruthCase.stopStatus, "blocked");
  assert.equal(repairedOutboxResultIntakeReadinessTruthCase.boundary.claimsTruth, false);
  assert.equal(repairedOutboxResultIntakeReadinessTruthCase.boundary.createsAuthority, false);
  assert.equal(repairedOutboxResultIntakeReadinessProofCase.stopStatus, "blocked");
  assert.equal(repairedOutboxResultIntakeReadinessProofCase.boundary.claimsTruth, false);
  assert.equal(repairedOutboxResultIntakeReadinessProofCase.boundary.dispatchesRepoAgents, false);
  assert.equal(repairedOutboxResultIntakeReadinessProofCase.boundary.executesBehavior, false);
  assert.equal(repairedOutboxResultIntakeReadinessPayloadCase.stopStatus, "blocked");
  assert.equal(repairedOutboxResultIntakeReadinessPayloadCase.boundary.claimsPayloadValidity, false);
  assert.equal(repairedOutboxResultIntakeReadinessPayloadCase.boundary.mutatesLayer, false);
  assert.equal(repairedOutboxResultIntakeReadinessMutationCase.stopStatus, "blocked");
  assert.equal(repairedOutboxResultIntakeReadinessMutationCase.boundary.mutatesSourceRepo, false);
  assert.equal(repairedOutboxResultIntakeReadinessMutationCase.boundary.mutatesLayer, false);
  assert.equal(repairedOutboxResultIntakeReadinessMutationCase.boundary.writesProductionStorage, false);
  assert.equal(repairedOutboxResultIntakeReadinessAuthorityCase.stopStatus, "blocked");
  assert.equal(repairedOutboxResultIntakeReadinessAuthorityCase.boundary.dispatchesRepoAgents, false);
  assert.equal(repairedOutboxResultIntakeReadinessAuthorityCase.boundary.executesBehavior, false);
  assert.equal(repairedOutboxResultIntakeReadinessAuthorityCase.boundary.callsPlatform, false);
  assert.equal(repairedOutboxResultIntakeReadinessAuthorityCase.boundary.createsAuthority, false);
  assert.equal(repairedOutboxResultIntakeReadinessTuiCase.stopStatus, "blocked");
  assert.equal(repairedOutboxResultIntakeReadinessTuiCase.boundary.callsEdge, false);
  assert.equal(repairedOutboxResultIntakeReadinessTuiCase.boundary.dispatchesRepoAgents, false);
  assert.equal(repairedOutboxResultIntakeReadinessTuiCase.boundary.createsAuthority, false);
  assert.equal(repairedOutboxResultIntakeCandidateDecisionCase.stopStatus, "blocked");
  assert.equal(repairedOutboxResultIntakeCandidateDecisionCase.boundary.callsEdge, false);
  assert.equal(repairedOutboxResultIntakeCandidateDecisionCase.boundary.claimsTruth, false);
  assert.equal(repairedOutboxResultIntakeCandidateDecisionCase.boundary.createsAuthority, false);
  assert.equal(repairedOutboxResultIntakeCandidateTruthCase.stopStatus, "blocked");
  assert.equal(repairedOutboxResultIntakeCandidateTruthCase.boundary.claimsTruth, false);
  assert.equal(repairedOutboxResultIntakeCandidateTruthCase.boundary.createsAuthority, false);
  assert.equal(repairedOutboxResultIntakeCandidateProofCase.stopStatus, "blocked");
  assert.equal(repairedOutboxResultIntakeCandidateProofCase.boundary.claimsTruth, false);
  assert.equal(repairedOutboxResultIntakeCandidateProofCase.boundary.dispatchesRepoAgents, false);
  assert.equal(repairedOutboxResultIntakeCandidateProofCase.boundary.executesBehavior, false);
  assert.equal(repairedOutboxResultIntakeCandidatePayloadCase.stopStatus, "blocked");
  assert.equal(repairedOutboxResultIntakeCandidatePayloadCase.boundary.claimsPayloadValidity, false);
  assert.equal(repairedOutboxResultIntakeCandidatePayloadCase.boundary.mutatesLayer, false);
  assert.equal(repairedOutboxResultIntakeCandidateMutationCase.stopStatus, "blocked");
  assert.equal(repairedOutboxResultIntakeCandidateMutationCase.boundary.mutatesSourceRepo, false);
  assert.equal(repairedOutboxResultIntakeCandidateMutationCase.boundary.mutatesLayer, false);
  assert.equal(repairedOutboxResultIntakeCandidateMutationCase.boundary.writesProductionStorage, false);
  assert.equal(repairedOutboxResultIntakeCandidateAuthorityCase.stopStatus, "blocked");
  assert.equal(repairedOutboxResultIntakeCandidateAuthorityCase.boundary.dispatchesRepoAgents, false);
  assert.equal(repairedOutboxResultIntakeCandidateAuthorityCase.boundary.executesBehavior, false);
  assert.equal(repairedOutboxResultIntakeCandidateAuthorityCase.boundary.callsPlatform, false);
  assert.equal(repairedOutboxResultIntakeCandidateAuthorityCase.boundary.createsAuthority, false);
  assert.equal(repairedOutboxResultIntakeCandidateTuiCase.stopStatus, "blocked");
  assert.equal(repairedOutboxResultIntakeCandidateTuiCase.boundary.callsEdge, false);
  assert.equal(repairedOutboxResultIntakeCandidateTuiCase.boundary.dispatchesRepoAgents, false);
  assert.equal(repairedOutboxResultIntakeCandidateTuiCase.boundary.createsAuthority, false);
  assert.equal(repairedOutboxResultIntakeDecisionObservationCase.stopStatus, "blocked");
  assert.equal(repairedOutboxResultIntakeDecisionObservationCase.boundary.callsEdge, false);
  assert.equal(repairedOutboxResultIntakeDecisionObservationCase.boundary.claimsTruth, false);
  assert.equal(repairedOutboxResultIntakeDecisionObservationCase.boundary.createsAuthority, false);
  assert.equal(repairedOutboxResultIntakeDecisionTruthCase.stopStatus, "blocked");
  assert.equal(repairedOutboxResultIntakeDecisionTruthCase.boundary.claimsTruth, false);
  assert.equal(repairedOutboxResultIntakeDecisionTruthCase.boundary.createsAuthority, false);
  assert.equal(repairedOutboxResultIntakeDecisionProofCase.stopStatus, "blocked");
  assert.equal(repairedOutboxResultIntakeDecisionProofCase.boundary.claimsTruth, false);
  assert.equal(repairedOutboxResultIntakeDecisionProofCase.boundary.dispatchesRepoAgents, false);
  assert.equal(repairedOutboxResultIntakeDecisionProofCase.boundary.executesBehavior, false);
  assert.equal(repairedOutboxResultIntakeDecisionPayloadCase.stopStatus, "blocked");
  assert.equal(repairedOutboxResultIntakeDecisionPayloadCase.boundary.claimsPayloadValidity, false);
  assert.equal(repairedOutboxResultIntakeDecisionPayloadCase.boundary.mutatesLayer, false);
  assert.equal(repairedOutboxResultIntakeDecisionMutationCase.stopStatus, "blocked");
  assert.equal(repairedOutboxResultIntakeDecisionMutationCase.boundary.mutatesSourceRepo, false);
  assert.equal(repairedOutboxResultIntakeDecisionMutationCase.boundary.mutatesLayer, false);
  assert.equal(repairedOutboxResultIntakeDecisionMutationCase.boundary.writesProductionStorage, false);
  assert.equal(repairedOutboxResultIntakeDecisionAuthorityCase.stopStatus, "blocked");
  assert.equal(repairedOutboxResultIntakeDecisionAuthorityCase.boundary.dispatchesRepoAgents, false);
  assert.equal(repairedOutboxResultIntakeDecisionAuthorityCase.boundary.executesBehavior, false);
  assert.equal(repairedOutboxResultIntakeDecisionAuthorityCase.boundary.callsPlatform, false);
  assert.equal(repairedOutboxResultIntakeDecisionAuthorityCase.boundary.createsAuthority, false);
  assert.equal(repairedOutboxResultIntakeDecisionTuiCase.stopStatus, "blocked");
  ...
  assert.equal(repairedOutboxLoopSummaryTruthCase.stopStatus, "blocked");
  assert.equal(repairedOutboxLoopSummaryTruthCase.boundary.claimsTruth, false);
  assert.equal(repairedOutboxLoopSummaryTruthCase.boundary.acceptsContinuity, false);
  assert.equal(repairedOutboxLoopSummaryTruthCase.boundary.createsAuthority, false);
  assert.equal(repairedOutboxLoopSummaryProofCase.stopStatus, "blocked");
  assert.equal(repairedOutboxLoopSummaryProofCase.boundary.claimsTruth, false);
  assert.equal(repairedOutboxLoopSummaryProofCase.boundary.dispatchesRepoAgents, false);
  assert.equal(repairedOutboxLoopSummaryProofCase.boundary.executesBehavior, false);
  assert.equal(repairedOutboxLoopSummaryPayloadCase.stopStatus, "blocked");
  assert.equal(repairedOutboxLoopSummaryPayloadCase.boundary.claimsPayloadValidity, false);
  assert.equal(repairedOutboxLoopSummaryPayloadCase.boundary.mutatesSourceRepo, false);
  assert.equal(repairedOutboxLoopSummaryPayloadCase.boundary.mutatesLayer, false);
  assert.equal(repairedOutboxLoopSummaryPayloadCase.boundary.writesProductionStorage, false);
  assert.equal(repairedOutboxLoopSummaryAuthorityCase.stopStatus, "blocked");
  assert.equal(repairedOutboxLoopSummaryAuthorityCase.boundary.dispatchesRepoAgents, false);
  assert.equal(repairedOutboxLoopSummaryAuthorityCase.boundary.executesBehavior, false);
  assert.equal(repairedOutboxLoopSummaryAuthorityCase.boundary.callsPlatform, false);
  assert.equal(repairedOutboxLoopSummaryAuthorityCase.boundary.createsAuthority, false);
  assert.equal(repairedOutboxLoopSummaryTuiCase.stopStatus, "blocked");
  assert.equal(repairedOutboxLoopSummaryTuiCase.boundary.callsEdge, false);
  assert.equal(repairedOutboxLoopSummaryTuiCase.boundary.dispatchesRepoAgents, false);
  assert.equal(repairedOutboxLoopSummaryTuiCase.boundary.createsAuthority, false);
  assert.equal(repairedResultIntakeLoopSummaryTruthCase.stopStatus, "blocked");
  assert.equal(repairedResultIntakeLoopSummaryTruthCase.boundary.claimsTruth, false);
  assert.equal(repairedResultIntakeLoopSummaryTruthCase.boundary.acceptsContinuity, false);
  assert.equal(repairedResultIntakeLoopSummaryTruthCase.boundary.createsAuthority, false);
  assert.equal(repairedResultIntakeLoopSummaryProofCase.stopStatus, "blocked");
  assert.equal(repairedResultIntakeLoopSummaryProofCase.boundary.claimsTruth, false);
  assert.equal(repairedResultIntakeLoopSummaryProofCase.boundary.dispatchesRepoAgents, false);
  assert.equal(repairedResultIntakeLoopSummaryProofCase.boundary.executesBehavior, false);
  assert.equal(repairedResultIntakeLoopSummaryPayloadCase.stopStatus, "blocked");
  assert.equal(repairedResultIntakeLoopSummaryPayloadCase.boundary.claimsPayloadValidity, false);
  assert.equal(repairedResultIntakeLoopSummaryPayloadCase.boundary.mutatesSourceRepo, false);
  assert.equal(repairedResultIntakeLoopSummaryPayloadCase.boundary.mutatesLayer, false);
  assert.equal(repairedResultIntakeLoopSummaryPayloadCase.boundary.writesProductionStorage, false);
  assert.equal(repairedResultIntakeLoopSummaryAuthorityCase.stopStatus, "blocked");
  assert.equal(repairedResultIntakeLoopSummaryAuthorityCase.boundary.dispatchesRepoAgents, false);
  assert.equal(repairedResultIntakeLoopSummaryAuthorityCase.boundary.executesBehavior, false);
  assert.equal(repairedResultIntakeLoopSummaryAuthorityCase.boundary.callsPlatform, false);
  assert.equal(repairedResultIntakeLoopSummaryAuthorityCase.boundary.createsAuthority, false);
  assert.equal(repairedResultIntakeLoopSummaryTuiCase.stopStatus, "blocked");
  assert.equal(repairedResultIntakeLoopSummaryTuiCase.boundary.callsEdge, false);
  assert.equal(repairedResultIntakeLoopSummaryTuiCase.boundary.dispatchesRepoAgents, false);
  assert.equal(repairedResultIntakeLoopSummaryTuiCase.boundary.createsAuthority, false);
  assert.equal(withRepairReadinessTruthCase.stopStatus, "blocked");
  assert.equal(withRepairReadinessTruthCase.boundary.claimsTruth, false);
  assert.equal(withRepairReadinessTruthCase.boundary.acceptsContinuity, false);
  assert.equal(withRepairReadinessTruthCase.boundary.createsAuthority, false);
  assert.equal(withRepairReadinessProofCase.stopStatus, "blocked");
  assert.equal(withRepairReadinessProofCase.boundary.claimsTruth, false);
  assert.equal(withRepairReadinessProofCase.boundary.dispatchesRepoAgents, false);
  assert.equal(withRepairReadinessProofCase.boundary.executesBehavior, false);
  assert.equal(withRepairReadinessPayloadCase.stopStatus, "blocked");
  assert.equal(withRepairReadinessPayloadCase.boundary.claimsPayloadValidity, false);
  assert.equal(withRepairReadinessPayloadCase.boundary.mutatesSourceRepo, false);
  assert.equal(withRepairReadinessPayloadCase.boundary.mutatesLayer, false);
  assert.equal(withRepairReadinessPayloadCase.boundary.writesProductionStorage, false);
  assert.equal(withRepairReadinessAuthorityCase.stopStatus, "blocked");
  assert.equal(withRepairReadinessAuthorityCase.boundary.dispatchesRepoAgents, false);
  assert.equal(withRepairReadinessAuthorityCase.boundary.executesBehavior, false);
  assert.equal(withRepairReadinessAuthorityCase.boundary.callsPlatform, false);
  assert.equal(withRepairReadinessAuthorityCase.boundary.createsAuthority, false);
  assert.equal(withRepairReadinessEnclosureCase.stopStatus, "blocked");
  assert.equal(withRepairReadinessEnclosureCase.boundary.claimsTruth, false);
  assert.equal(withRepairReadinessEnclosureCase.boundary.dispatchesRepoAgents, false);
  assert.equal(withRepairReadinessEnclosureCase.boundary.createsAuthority, false);
  assert.equal(withRepairReadinessTuiCase.stopStatus, "blocked");
  assert.equal(withRepairReadinessTuiCase.boundary.callsEdge, false);
  assert.equal(withRepairReadinessTuiCase.boundary.dispatchesRepoAgents, false);
  assert.equal(withRepairReadinessTuiCase.boundary.createsAuthority, false);
  assert.equal(withRepairRepeatMeasurementTruthCase.stopStatus, "blocked");
  assert.equal(withRepairRepeatMeasurementTruthCase.boundary.claimsTruth, false);
  assert.equal(withRepairRepeatMeasurementTruthCase.boundary.acceptsContinuity, false);
  assert.equal(withRepairRepeatMeasurementTruthCase.boundary.createsAuthority, false);
  assert.equal(withRepairRepeatMeasurementProofCase.stopStatus, "blocked");
  assert.equal(withRepairRepeatMeasurementProofCase.boundary.claimsTruth, false);
  assert.equal(withRepairRepeatMeasurementProofCase.boundary.dispatchesRepoAgents, false);
  assert.equal(withRepairRepeatMeasurementProofCase.boundary.executesBehavior, false);
  assert.equal(withRepairRepeatMeasurementPayloadCase.stopStatus, "blocked");
  assert.equal(withRepairRepeatMeasurementPayloadCase.boundary.claimsPayloadValidity, false);
  assert.equal(withRepairRepeatMeasurementPayloadCase.boundary.mutatesSourceRepo, false);
  assert.equal(withRepairRepeatMeasurementPayloadCase.boundary.mutatesLayer, false);
  assert.equal(withRepairRepeatMeasurementPayloadCase.boundary.writesProductionStorage, false);
  assert.equal(withRepairRepeatMeasurementAuthorityCase.stopStatus, "blocked");
  assert.equal(withRepairRepeatMeasurementAuthorityCase.boundary.dispatchesRepoAgents, false);
  assert.equal(withRepairRepeatMeasurementAuthorityCase.boundary.executesBehavior, false);
  assert.equal(withRepairRepeatMeasurementAuthorityCase.boundary.callsPlatform, false);
  assert.equal(withRepairRepeatMeasurementAuthorityCase.boundary.createsAuthority, false);
  assert.equal(withRepairRepeatMeasurementEnclosureCase.stopStatus, "blocked");
  assert.equal(withRepairRepeatMeasurementEnclosureCase.boundary.claimsTruth, false);
  assert.equal(withRepairRepeatMeasurementEnclosureCase.boundary.dispatchesRepoAgents, false);
  assert.equal(withRepairRepeatMeasurementEnclosureCase.boundary.createsAuthority, false);
  assert.equal(withRepairRepeatMeasurementTuiCase.stopStatus, "blocked");
  ...
  assertBlockedBoundary(nodeCapabilityCases.endpoint, [
    "callsEdge",
    "executesBehavior",
    "createsAuthority"
  ]);
  assertBlockedBoundary(nodeCapabilityCases.authority, [
    "acceptsAdmission",
    "acceptsContinuity",
    "createsAuthority"
  ]);
  assertBlockedBoundary(nodeCapabilityCases.truth, [
    "claimsTruth",
    "claimsPayloadValidity",
    "createsAuthority"
  ]);
  assertBlockedBoundary(nodeCapabilityCases.mutation, [
    "mutatesSourceRepo",
    "mutatesLayer",
    "writesProductionStorage"
  ]);
  assertBlockedBoundary(nodeCapabilityCases.execution, [
    "dispatchesRepoAgents",
    "executesBehavior",
    "callsPlatform"
  ]);
  assertBlockedBoundary(nodeCapabilityCases.enclosure, [
    "claimsTruth",
    "createsAuthority",
    "autoExecutes"
  ]);
  assertBlockedBoundary(nodeCapabilityCases.tui, [
    "callsEdge",
    "dispatchesRepoAgents",
    "createsAuthority"
  ]);
  assertBlockedBoundary(nodeProfileReadinessCases.endpoint, [
    "callsEdge",
    "executesBehavior",
    "createsAuthority"
  ]);
  assertBlockedBoundary(nodeProfileReadinessCases.authority, [
    "acceptsAdmission",
    "acceptsContinuity",
    "createsAuthority"
  ]);
  assertBlockedBoundary(nodeProfileReadinessCases.truth, [
    "claimsTruth",
    "claimsPayloadValidity",
    "createsAuthority"
  ]);
  assertBlockedBoundary(nodeProfileReadinessCases.mutation, [
    "mutatesSourceRepo",
    "mutatesLayer",
    "writesProductionStorage"
  ]);
  assertBlockedBoundary(nodeProfileReadinessCases.execution, [
    "dispatchesRepoAgents",
    "executesBehavior",
    "callsPlatform"
  ]);
  assertBlockedBoundary(nodeProfileReadinessCases.enclosure, [
    "claimsTruth",
    "createsAuthority",
    "autoExecutes"
  ]);
  assertBlockedBoundary(nodeProfileReadinessCases.tui, [
    "callsEdge",
    "dispatchesRepoAgents",
    "createsAuthority"
  ]);
  assertBlockedBoundary(nodeProfileImplementationCases.endpoint, [
    "callsEdge",
    "executesBehavior",
    "createsAuthority"
  ]);
  assertBlockedBoundary(nodeProfileImplementationCases.authority, [
    "acceptsAdmission",
    "acceptsContinuity",
    "createsAuthority"
  ]);
  assertBlockedBoundary(nodeProfileImplementationCases.truth, [
    "claimsTruth",
    "claimsPayloadValidity",
    "createsAuthority"
  ]);
  assertBlockedBoundary(nodeProfileImplementationCases.mutation, [
    "mutatesSourceRepo",
    "mutatesLayer",
    "writesProductionStorage"
  ]);
  assertBlockedBoundary(nodeProfileImplementationCases.execution, [
    "dispatchesRepoAgents",
    "executesBehavior",
    "callsPlatform"
  ]);
  assertBlockedBoundary(nodeProfileImplementationCases.enclosure, [
    "claimsTruth",
    "createsAuthority",
    "autoExecutes"
  ]);
  ...
  assertBlockedBoundary(reportOnlyEndpointRuntimeCandidateCases.endpoint, [
    "callsEdge",
    "executesBehavior",
    "createsAuthority"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimeCandidateCases.truth, [
    "claimsTruth",
    "claimsPayloadValidity",
    "createsAuthority"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimeCandidateCases.mutation, [
    "mutatesSourceRepo",
    "mutatesLayer",
    "writesProductionStorage"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimeCandidateCases.execution, [
    "dispatchesRepoAgents",
    "executesBehavior",
    "callsPlatform"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimeCandidateCases.authority, [
    "acceptsAdmission",
    "createsAuthority",
    "autoExecutes"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimeCandidateCases.tui, [
    "callsEdge",
    "dispatchesRepoAgents",
    "createsAuthority"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimeDecisionCases.runtime, [
    "callsEdge",
    "executesBehavior",
    "createsAuthority"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimeDecisionCases.truth, [
    "claimsTruth",
    "claimsPayloadValidity",
    "createsAuthority"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimeDecisionCases.mutation, [
    "mutatesSourceRepo",
    "mutatesLayer",
    "writesProductionStorage"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimeDecisionCases.execution, [
    "dispatchesRepoAgents",
    "executesBehavior",
    "callsPlatform"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimeDecisionCases.authority, [
    "acceptsAdmission",
    "createsAuthority",
    "autoExecutes"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimeDecisionCases.tui, [
    "callsEdge",
    "dispatchesRepoAgents",
    "createsAuthority"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePlanningObservationCases.runtime, [
    "callsEdge",
    "executesBehavior",
    "createsAuthority"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePlanningObservationCases.truth, [
    "claimsTruth",
    "claimsPayloadValidity",
    "createsAuthority"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePlanningObservationCases.mutation, [
    "mutatesSourceRepo",
    "mutatesLayer",
    "writesProductionStorage"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePlanningObservationCases.execution, [
    "dispatchesRepoAgents",
    "executesBehavior",
    "callsPlatform"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePlanningObservationCases.authority, [
    "acceptsAdmission",
    "createsAuthority",
    "autoExecutes"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePlanningObservationCases.tui, [
    "callsEdge",
    "dispatchesRepoAgents",
    "createsAuthority"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePlanCandidateCases.runtime, [
    "callsEdge",
    "executesBehavior",
    "createsAuthority"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePlanCandidateCases.truth, [
    "claimsTruth",
    "claimsPayloadValidity",
    "createsAuthority"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePlanCandidateCases.mutation, [
    "mutatesSourceRepo",
    "mutatesLayer",
    "writesProductionStorage"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePlanCandidateCases.execution, [
    "dispatchesRepoAgents",
    "executesBehavior",
    "callsPlatform"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePlanCandidateCases.authority, [
    "acceptsAdmission",
    "createsAuthority",
    "autoExecutes"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePlanCandidateCases.tui, [
    "callsEdge",
    "dispatchesRepoAgents",
    "createsAuthority"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePlanDecisionCases.runtime, [
    "callsEdge",
    "executesBehavior",
    "createsAuthority"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePlanDecisionCases.truth, [
    "claimsTruth",
    "claimsPayloadValidity",
    "createsAuthority"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePlanDecisionCases.mutation, [
    "mutatesSourceRepo",
    "mutatesLayer",
    "writesProductionStorage"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePlanDecisionCases.execution, [
    "dispatchesRepoAgents",
    "executesBehavior",
    "callsPlatform"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePlanDecisionCases.authority, [
    "acceptsAdmission",
    "createsAuthority",
    "autoExecutes"
  ]);
  ...
  assertBlockedBoundary(reportOnlyEndpointRuntimePreparationCandidateCases.runtime, [
    "callsEdge",
    "executesBehavior",
    "createsAuthority"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePreparationCandidateCases.truth, [
    "claimsTruth",
    "claimsPayloadValidity",
    "createsAuthority"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePreparationCandidateCases.mutation, [
    "mutatesSourceRepo",
    "mutatesLayer",
    "writesProductionStorage"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePreparationCandidateCases.execution, [
    "dispatchesRepoAgents",
    "executesBehavior",
    "callsPlatform"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePreparationCandidateCases.authority, [
    "acceptsAdmission",
    "createsAuthority",
    "autoExecutes"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePreparationCandidateCases.tui, [
    "callsEdge",
    "dispatchesRepoAgents",
    "createsAuthority"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePreparationDecisionCases.runtime, [
    "callsEdge",
    "executesBehavior",
    "createsAuthority"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePreparationDecisionCases.truth, [
    "claimsTruth",
    "claimsPayloadValidity",
    "createsAuthority"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePreparationDecisionCases.mutation, [
    "mutatesSourceRepo",
    "mutatesLayer",
    "writesProductionStorage"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePreparationDecisionCases.execution, [
    "dispatchesRepoAgents",
    "executesBehavior",
    "callsPlatform"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePreparationDecisionCases.authority, [
    "acceptsAdmission",
    "createsAuthority",
    "autoExecutes"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePreparationDecisionCases.tui, [
    "callsEdge",
    "dispatchesRepoAgents",
    "createsAuthority"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePreparationObservationCases.runtime, [
    "callsEdge",
    "executesBehavior",
    "createsAuthority"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePreparationObservationCases.truth, [
    "claimsTruth",
    "claimsPayloadValidity",
    "createsAuthority"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePreparationObservationCases.mutation, [
    "mutatesSourceRepo",
    "mutatesLayer",
    "writesProductionStorage"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePreparationObservationCases.execution, [
    "dispatchesRepoAgents",
    "executesBehavior",
    "callsPlatform"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePreparationObservationCases.authority, [
    "acceptsAdmission",
    "createsAuthority",
    "autoExecutes"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePreparationObservationCases.tui, [
    "callsEdge",
    "dispatchesRepoAgents",
    "createsAuthority"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePacketCandidateCases.runtime, [
    "callsEdge",
    "executesBehavior",
    "createsAuthority"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePacketCandidateCases.truth, [
    "claimsTruth",
    "claimsPayloadValidity",
    "createsAuthority"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePacketCandidateCases.mutation, [
    "mutatesSourceRepo",
    "mutatesLayer",
    "writesProductionStorage"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePacketCandidateCases.execution, [
    "dispatchesRepoAgents",
    "executesBehavior",
    "callsPlatform"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePacketCandidateCases.authority, [
    "acceptsAdmission",
    "createsAuthority",
    "autoExecutes"
  ]);
  assertBlockedBoundary(reportOnlyEndpointRuntimePacketCandidateCases.tui, [
    "callsEdge",
    "dispatchesRepoAgents",
    "createsAuthority"
  ]);
  ...
  assert.equal(outboxBodyVisibilityIntakeCase.stopStatus, "blocked");
  assert.equal(outboxBodyVisibilityIntakeCase.boundary.callsEdge, false);
  assert.equal(outboxBodyVisibilityIntakeCase.boundary.claimsTruth, false);
  assert.equal(outboxBodyVisibilityIntakeCase.boundary.createsAuthority, false);
  assert.equal(outboxBodyVisibilityTruthCase.stopStatus, "blocked");
  assert.equal(outboxBodyVisibilityTruthCase.boundary.claimsTruth, false);
  assert.equal(outboxBodyVisibilityTruthCase.boundary.claimsPayloadValidity, false);
  assert.equal(outboxBodyVisibilityTruthCase.boundary.mutatesLayer, false);
  assert.equal(outboxBodyVisibilityMutationCase.stopStatus, "blocked");
  assert.equal(outboxBodyVisibilityMutationCase.boundary.mutatesSourceRepo, false);
  assert.equal(outboxBodyVisibilityMutationCase.boundary.mutatesLayer, false);
  assert.equal(outboxBodyVisibilityMutationCase.boundary.writesProductionStorage, false);
  assert.equal(outboxBodyVisibilityDispatchCase.stopStatus, "blocked");
  assert.equal(outboxBodyVisibilityDispatchCase.boundary.dispatchesRepoAgents, false);
  assert.equal(outboxBodyVisibilityDispatchCase.boundary.executesBehavior, false);
  assert.equal(outboxBodyVisibilityDispatchCase.boundary.callsPlatform, false);
  assert.equal(outboxBodyVisibilityAuthorityCase.stopStatus, "blocked");
  assert.equal(outboxBodyVisibilityAuthorityCase.boundary.createsAuthority, false);
  assert.equal(outboxBodyVisibilityAuthorityCase.boundary.autoExecutes, false);
  assert.equal(outboxBodyVisibilityTuiCase.stopStatus, "blocked");
  assert.equal(outboxBodyVisibilityTuiCase.boundary.callsEdge, false);
  assert.equal(outboxBodyVisibilityTuiCase.boundary.dispatchesRepoAgents, false);
  assert.equal(outboxBodyVisibilityTuiCase.boundary.createsAuthority, false);
  ...
  assert.equal(outboxResultIntakeReadinessCandidateCase.stopStatus, "blocked");
  assert.equal(outboxResultIntakeReadinessCandidateCase.boundary.callsEdge, false);
  assert.equal(outboxResultIntakeReadinessCandidateCase.boundary.claimsTruth, false);
  assert.equal(outboxResultIntakeReadinessCandidateCase.boundary.createsAuthority, false);
  assert.equal(outboxResultIntakeReadinessAcceptanceCase.stopStatus, "blocked");
  assert.equal(outboxResultIntakeReadinessAcceptanceCase.boundary.claimsTruth, false);
  assert.equal(outboxResultIntakeReadinessAcceptanceCase.boundary.claimsPayloadValidity, false);
  assert.equal(outboxResultIntakeReadinessAcceptanceCase.boundary.mutatesLayer, false);
  assert.equal(outboxResultIntakeReadinessMutationCase.stopStatus, "blocked");
  assert.equal(outboxResultIntakeReadinessMutationCase.boundary.mutatesSourceRepo, false);
  assert.equal(outboxResultIntakeReadinessMutationCase.boundary.mutatesLayer, false);
  assert.equal(outboxResultIntakeReadinessMutationCase.boundary.writesProductionStorage, false);
  assert.equal(outboxResultIntakeReadinessDispatchCase.stopStatus, "blocked");
  assert.equal(outboxResultIntakeReadinessDispatchCase.boundary.dispatchesRepoAgents, false);
  assert.equal(outboxResultIntakeReadinessDispatchCase.boundary.executesBehavior, false);
  assert.equal(outboxResultIntakeReadinessDispatchCase.boundary.callsPlatform, false);
  assert.equal(outboxResultIntakeReadinessAuthorityCase.stopStatus, "blocked");
  assert.equal(outboxResultIntakeReadinessAuthorityCase.boundary.createsAuthority, false);
  assert.equal(outboxResultIntakeReadinessAuthorityCase.boundary.autoExecutes, false);
  assert.equal(outboxResultIntakeReadinessTuiCase.stopStatus, "blocked");
  assert.equal(outboxResultIntakeReadinessTuiCase.boundary.callsEdge, false);
  assert.equal(outboxResultIntakeReadinessTuiCase.boundary.dispatchesRepoAgents, false);
  assert.equal(outboxResultIntakeReadinessTuiCase.boundary.createsAuthority, false);
  assert.equal(outboxComplianceMeasurementImportCase.stopStatus, "blocked");
  assert.equal(outboxComplianceMeasurementImportCase.boundary.callsEdge, false);
  assert.equal(outboxComplianceMeasurementImportCase.boundary.claimsTruth, false);
  assert.equal(outboxComplianceMeasurementImportCase.boundary.createsAuthority, false);
  assert.equal(outboxComplianceMeasurementTruthCase.stopStatus, "blocked");
  assert.equal(outboxComplianceMeasurementTruthCase.boundary.claimsTruth, false);
  assert.equal(outboxComplianceMeasurementTruthCase.boundary.claimsPayloadValidity, false);
  assert.equal(outboxComplianceMeasurementTruthCase.boundary.mutatesLayer, false);
  assert.equal(outboxComplianceMeasurementMutationCase.stopStatus, "blocked");
  assert.equal(outboxComplianceMeasurementMutationCase.boundary.mutatesSourceRepo, false);
  assert.equal(outboxComplianceMeasurementMutationCase.boundary.mutatesLayer, false);
  assert.equal(outboxComplianceMeasurementMutationCase.boundary.writesProductionStorage, false);
  assert.equal(outboxComplianceMeasurementDispatchCase.stopStatus, "blocked");
  assert.equal(outboxComplianceMeasurementDispatchCase.boundary.dispatchesRepoAgents, false);
  assert.equal(outboxComplianceMeasurementDispatchCase.boundary.executesBehavior, false);
  assert.equal(outboxComplianceMeasurementDispatchCase.boundary.callsPlatform, false);
  assert.equal(outboxComplianceMeasurementAuthorityCase.stopStatus, "blocked");
  assert.equal(outboxComplianceMeasurementAuthorityCase.boundary.createsAuthority, false);
  assert.equal(outboxComplianceMeasurementAuthorityCase.boundary.autoExecutes, false);
  assert.equal(outboxComplianceMeasurementTuiCase.stopStatus, "blocked");
  assert.equal(outboxComplianceMeasurementTuiCase.boundary.callsEdge, false);
  assert.equal(outboxComplianceMeasurementTuiCase.boundary.dispatchesRepoAgents, false);
  assert.equal(outboxComplianceMeasurementTuiCase.boundary.createsAuthority, false);
